...
📦 **下载安装**：[shiyi-agent-v2.6.6.apk](https://github.com/JIUSIS/shiyi-agent/releases/download/v2.6.6/shiyi-agent-v2.6.6.apk) · [更新日志](CHANGELOG.md) · [GitHub Releases](https://github.com/JIUSIS/shiyi-agent/releases)
...
拾忆接入 [LAAP（Living Agent Application Protocol）意识工程](https://github.com/lorryjovens-hub/laap-AGI)。LAAP 是面向数字生命体的 Zero-LLM 认知架构：由 PSI Core、情绪与需求状态、情景记忆、因果推理和规则引擎等认知模块提供持续的内在状态，让大语言模型成为表达与交流伙伴，而不只是单独承担全部认知工作。

在拾忆中，LAAP 作为独立的本地认知皮层运行，通过认知状态、记忆召回与回合反思生成动态认知上下文，再交给当前模型参与对话。它不替代拾忆的聊天引擎，也不改变模型接口；“活人感”默认关闭，用户可在 Agent 引擎页单独启用。
...
- DeepSeek Harness 工作区：会话只能在本工作区内长按排序；本机 / 局域网 / 公网都不支持把同一条会话换到另一个工作区（官方 cwd 写死在日志头）
...
- 群聊：拾忆主页会话 tab 与功能页可进；会话页与拾忆聊天同一套液态玻璃输入框和气泡，卡片左滑编辑/删除。可贴 mermaid / 字符思维导图生成 Agent。多个独立 Agent 各自配置接口、名字、职位和人设；按公司结构对接，每个人的思考过程单独可见。不带官方会话工具，仅拾忆
- 网页搜索与网页内容提取，多来源交叉验证
...
本项目在 [LINUX DO](https://linux.do) 社区交流反馈，感谢佬友们的支持。

---

**拾忆 ShiYi** · 让每一次对话，都留下成果。

> 本项目在 [LINUX DO](https://linux.do) 和[LINUX SB](https://linux.sb)社区交流反馈。
> 我的主页 [九思JIUSIS](https://linux.do/u/jiusis)和[九思](https://linux.sb/user/9466)
