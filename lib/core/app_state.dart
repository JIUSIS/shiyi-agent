...
class ShiyiState extends ChangeNotifier {
  ...
  Future<String> _buildSystemPrompt(
    ...
    return assemblePromptSections(
      _buildPromptSections(userText, rollingSummary: rollingSummary),
    );
  }
  ...
  List<PromptSection> _buildPromptSections(
    String userText, {
    String rollingSummary = '',
  }) => [
    PromptSection(
    ...
    if (settings.enableMemory)
      PromptSection(
        ...
        order: 300,
        builder: () async {
          final memCtx = await _db.recentMemoriesWithTerms(
            _keywords(userText),
            8,
          );
  ...
  Future<Map<String, String>> _buildPromptVariables(String userText) async {
    final now = DateTime.now();
    return {
      'model': settings.model.isEmpty ? '未配置模型' : settings.model,
