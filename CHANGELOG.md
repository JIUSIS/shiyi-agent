...
相对 2.6.1：子代理可以点开看 mini 会话；缓存和子代理收成输入框上方的小按钮；项目目录改成文件夹图标。修掉点子代理红屏、返回红屏、连点叠栏，以及主 agent 等待时子代理状态被清掉的问题。
...
- `flutter analyze` 无新增 error；`flutter test` 859 项全绿；`flutter build apk --release --target-platform android-arm64` 生成 18.4MB `app-release.apk`（仅 arm64-v8a）。
- 详细记录见 `docs/fix-log.md` #318–#323。
