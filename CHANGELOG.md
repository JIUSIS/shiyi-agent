...
- 修复 DSH 新版会话历史读取为空：按官方流程先从 `session/follow` 获取当前游标，再读取完整 `session/page` 历史。
- 修复 DSH 启动期间缓存会话只显示旧消息的问题；服务恢复后会自动补读权威历史，助手正文、思考和用户消息不再错序或互相覆盖。
...
- DSH、LAAP 定向测试通过；DSH 相关测试 148 项全绿。
- 群聊认知注入回归测试通过。
- Android arm64 release APK 构建成功，版本号为 `2.6.6 (31)`。
- 发布包使用标准 Flutter release 产物，APK 大小以本次正式构建结果为准。
...
- 修复拾忆历史里混入「无思考内容的助手消息」时，DeepSeek thinking 网关拒绝请求的问题：请求组包时统一给缺失 `reasoning_content` 的 assistant 补空字段。
- 修复输入框上方抽屉（模型 / 思考强度 / 权限预设 / 缓存统计）在键盘收起后仍停留在旧位置的问题：抽屉会跟随按钮最新位置，并监听键盘高度变化在收起过程中持续重定位。

### 验证

- `flutter analyze` 无新增 error；`flutter test` 全量 904 项全绿。
- `flutter build apk --release --target-platform android-arm64` 成功，仅 `arm64-v8a`。
- 真机 `adb install -r` 覆盖安装 Success，不卸载、不清数据。

## [2.6.4] - 2026-09-02

相对 2.6.3：群聊成员获得真实工作目录和文件 / 终端工具，能真正产出文件；群聊输入区对齐普通会话，并补齐每个成员的独立上下文压缩；运行中返回不再打断输出，思考过程展开后自动追最新。

### 新增

- **群聊文件与终端工具**：每个成员都有 `file_read` / `file_write` / `run_terminal`，读写基于项目工作目录；不再“只说不做”，任务推进会落成真实文件。
- **群聊独立上下文压缩**：每个 Agent 单独生成历史摘要并随请求注入；历史消息按「点名 / 上级 / 直属下属」过滤相关上下文，避免全员互相污染。
- **群聊输入区对齐普通会话**：统一 API 配置抽屉、附件、项目文件夹入口、整体思考开关与强度，以及每个成员单独的上下文压缩入口。
- **文本菜单中文化**：接入 zh/en 本地化委托，长按复制 / 粘贴等菜单不再全是英文。

### 优化

- **后台续跑**：群聊运行中按返回不再打断输出，任务在后台跑完并落库；重新进入能看到实时草稿和正确的运行状态。
- **思考自动追新**：群聊思考过程展开后，流式增长自动滚到最新。
- **子代理跟随**：子代理 mini 会话上滑看历史不被流式拽回，只有贴底时才跟随最新。
- **输入框失焦**：点击输入框外轻点才释放焦点，长按、拖动选择、复制工具条不再一收一弹。

### 修复

- 修复群聊历史没回传 `reasoning_content` 导致的 `LITELLM_ERROR 400`。
- 修复打开软件即误弹「DSH 未安装」：只有切到 DSH 引擎时才检查并提示。
- 修复群聊返回后「状态在运行但页面无实时输出」。
- 修复长按输入框内容时输入法一收一弹、复制不了的问题。

### 验证

- `flutter analyze` 无新增 error，仅保留仓库既有提示；`flutter test` 全量 903 项全绿。
- `flutter build apk --release --target-platform android-arm64` 成功，仅 `arm64-v8a`。
- 真机 `adb install -r` 覆盖安装 Success，不卸载、不清数据。
